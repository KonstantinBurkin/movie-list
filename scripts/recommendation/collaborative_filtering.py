...
class CollaborativeFilteringModel:
    ...
    def train(self, months_back: int = 6):
        ...
        if len(self.recent_movies) < 5:
            print(f"Warning: Only {len(self.recent_movies)} recent movies found. Need at least 5 for good recommendations.")

        self.extract_features()

        print("\nUser profile summary:")
        print(f"  Top genres: {sorted(self.movie_features['genres'].items(), key=lambda x: x[1], reverse=True)[:5]}")
        print(f"  Top directors: {sorted(self.movie_features['directors'].items(), key=lambda x: x[1], reverse=True)[:3]}")
